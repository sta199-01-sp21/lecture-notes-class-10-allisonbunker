...
i would guess the first is more likely

```{r simulate_rolls}

dice_roll_results<- tibble(
...
dice_roll_results<- dice_roll_results %>%
  mutate(one = roll_1^2 == roll_2 , 
         two = abs(roll_2-roll_1) == 4)
...
dice_roll_results %>%
  summarize(event_1_prob = mean(roll_1),
            event_2_prob = mean(roll_2))
